...
class Cms extends BaseController
{
	...
	/**
	* This function is used to load the Home CMS content list
	*/
	
	public function home_cms($type = 0)
	{
		$data=array();
		$type1 = $type;
		if ($type1 == 0) {
			$data['title']='Home CMS List';
		} else if ($type1 == 1) {
			...
			$data['title']='Payment Methods Icons';
		} else if ($type1 == 5) {
			$data['title']='About Us';
		}
		...
		$this->db->select('tbl_homecms.*');
		$this->db->from('tbl_homecms');
		$this->db->where('tbl_homecms.type',$type);
		$query = $this->db->get();
		$data['list']=$query->result_array();

		if ($type1 == 0) {
			$this->loadViews("cms/home_cms", $this->global, $data);
		} else if ($type1 == 1) {
			$this->loadViews("cms/slider", $this->global, $data);
		} else if ($type1 == 2) {
			$this->loadViews("cms/graph", $this->global, $data);
		} else if ($type1 == 3) {
			$this->loadViews("cms/blog", $this->global, $data);
		} else if ($type1 == 4) {
			$this->loadViews("cms/payment_methods", $this->global, $data);
		} else if ($type1 == 5) {
			$this->loadViews("cms/about_us", $this->global, $data);
		}
		
	}

	public function edit_cms($id=0, $type=0)
	{
		$data['id'] = $id;
		$data['type'] = $type;
		...
		if ($id) {
			$this->db->select('tbl_homecms.*');
			$this->db->from('tbl_homecms');
			$this->db->where('tbl_homecms.id',$id);
			$this->db->where('tbl_homecms.type',$type);
			$query = $this->db->get();
			$data['details'] = $query->row();
		}
		if ($type == 3) {
			$data['page_content']= $this->loadViews("cms/blog_edit", $this->global, $data, true);
		} else if ($type == 4) {
			$data['page_content']= $this->loadViews("cms/edit", $this->global, $data, true);
		} else if ($type == 5) {
			$data['page_content']= $this->loadViews("cms/about_us_edit", $this->global, $data, true);
		} else {
			$data['page_content']= $this->loadViews("cms/edit", $this->global, $data, true);
		}
	...
	public function edit_graph($id=0, $type=0)
	{
		$data['id'] = $id;
		$data['type'] = $type;
		if ($id) {
		...
		$data['page_content']= $this->loadViews("cms/edit_graph", $this->global, $data, true);
	}

	public function home_cms_save($id = 0, $type = '')
	{
		$form=$_POST;
		...
		if (strlen($_FILES['img']['tmp_name']) != 0 ) {
			...
			$img_name = $image_array[0];
			$ext = $image_array[1];
			$name = $img_name.'_'.time().'.'.$ext;
			move_uploaded_file($_FILES['img']['tmp_name'], 'assets/dist/img/'.$name);
		}
		$data=array(
			'title' => $form['title'],
		...
		if ($id) {
			...
			if ($this->db->update('tbl_homecms')) {
				$reply['success']= true;
				$reply['message']='<span clas="text text-success">Data has been update successfully.</span>';
				echo json_encode($reply); die;
			};
		}
	}
	
	public function blog_save($id = 0, $type = '')
	{
		$form=$_POST;
		$data=array(
		'title' => $form['title'],
		...
		'datetime' => $form['date']
			...
			if ($this->db->update('tbl_homecms')) {
				$reply['success']= true;
				$reply['message']='Data has been update successfully.';
				echo json_encode($reply); die;
			};
	}
